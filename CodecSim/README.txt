...
The contents of this package are
  ...
  - requirements.txt : Requirement file (pip install requirements.txt)
  
  - codec_configure : Install audio codecs (sh codec_configure)	
  
  - generator.py : Degrades single file (.wav file)(-h argument for help)
  
  - degrade-audio-list-safe-random__.py : Degrades an audio file (provide list of paths in .txt format in arguments, -h for usage help)
  ...
  - prepare-impulse-responses__.py : Prepare and normalize impulse the packaged and downloaded impulses 
  ...
  - degrade-audio-safe-random__.py : Degrades a list of audio files under pre-specified degradation conditions (landline, cellular, satellite, interview, playback) along with noisy variants
