...
import glob
#print(os.getcwd()+ '/' + irOriginal + '/**/*.wav')
swav = glob.glob(os.getcwd()+ '/' + irOriginal + '/**/*.wav', recursive=True)
...
for ln in swav:
	...
	ir8k = signal.resample(ir, int(8000/float(fs)))
	ir8kabs = ir8k * ir8k
	sum8k=ir8kabs.sum()
	...
	ir16k = signal.resample(ir, int(8000/float(fs)))
